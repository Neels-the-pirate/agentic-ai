...
The repository also includes `render.yaml` with these settings. If you configure the service manually, make sure the Render **Root Directory** is `server` and the **Start Command** is `npm start`. Do not use `node start`.
